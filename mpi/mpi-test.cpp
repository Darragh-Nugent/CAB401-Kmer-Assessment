...
void CompareAllBacteria()
{
    ...
    for (int i = rank; i < number_bacteria; i += size)
    {
        for (int j = i + 1; j < number_bacteria; j++)
        {
            ...
            printf("%.20lf\n", correlation);
            correlations[(i * (number_bacteria - 1)) - ((i - 1) * i / 2) + (j - i - 1)] = correlation;
        }
    }
...
void WriteCorrelationsToFile(const char* output_name, double* corr_array) {
    ...
    for (int i = 0; i < number_bacteria - 1; i++) {
        for (int j = i + 1; j < number_bacteria; j++) {
            fprintf(output_file, "%2d %2d -> %.20lf\n", i, j, corr_array[index++]);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    correlations = new double[(number_bacteria * (number_bacteria - 1)) / 2];
    std::fill(correlations, correlations + (number_bacteria * (number_bacteria - 1)) / 2, 0.0);
    CompareAllBacteria();

    int total_corrs = (number_bacteria * (number_bacteria - 1)) / 2;
    double *final_correlations = nullptr;
    if (rank == 0)
    {
        final_correlations = new double[total_corrs];
    }

    MPI_Reduce(correlations, final_correlations, total_corrs, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);

    if (rank == 0)
    {
        WriteCorrelationsToFile("mpi-correlations.txt", final_correlations);
