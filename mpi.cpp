...
void RetrieveBacteriaInfo(int *all_counts, Bacteria **local_b, double **all_sig_t_vec, long **all_sig_t_indx_vec)
{
	...
	for (int i = 0; i < number_bacteria; i++)
	{
		...
		if (rank == 0)
		{
			if (owner_rank == 0)
			{
				all_counts[i] = local_b[i]->sig_t_vec.size();
		...
		{
			int count = local_b[i]->sig_t_vec.size();
			MPI_Send(&count, 1, MPI_INT, 0, tag_count, MPI_COMM_WORLD);
			MPI_Send(local_b[i]->sig_t_vec.data(), count, MPI_DOUBLE, 0, tag_vec, MPI_COMM_WORLD);
			MPI_Send(local_b[i]->sig_t_indx_vec.data(), count, MPI_LONG, 0, tag_indx, MPI_COMM_WORLD);
		}
	}
}
...
void CompareAllBacteria()
{
	...
	for (int i = 0; i < number_bacteria; i++)
	{
		if (i >= start && i < end)
		{
			printf("rank %d: load %d of %d\n", rank, i + 1, number_bacteria);
